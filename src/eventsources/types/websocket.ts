...
export default class WebSocketEventSource extends GSEventSource {
  ...
  protected async initClient(): Promise<PlainObject> {
    ...
    logger.info(`WebSocketEventSource listening on port ${port}`);
    ...
    this.wss.on('connection', (ws: ExtendedWebSocket, req) => {
      ...
      ws.on('message', (rawData: WebSocket.RawData) => {
        try {
          ...
          if (!eventtype || !schema) {
            ws.send(
              JSON.stringify({
                ...
                payload: {
                  message: `Unknown or unconfigured event type: ${eventtype}`,
                },
              }),
            );
            logger.error(`Unknown or unconfigured event type: ${eventtype}`);
            return;
          }
          ...
          logger.error('Zod Validation Error:', e.errors);
          ws.send(
            JSON.stringify({
              eventtype: 'stream.error',
              payload: { message: e.message || 'Invalid format' },
            }),
          );
        }
      });
  ...
  async subscribeToEvent(
    ...
    try {
      ...
      logger.info(`Schema registered for ${eventKey}`);
    } catch (err) {
      logger.error(`Failed to compile schema for ${eventKey}:`, err);
    }
    ...
    wsEventBus.on(
      ...
      async ({
        ...
        try {
          ...
          logger.error(`Error in handler for ${eventKey}`, err);
        }
      },
    );

    logger.info(`Event handler registered for ${eventKey}`);
  }
  ...
  private convertJsonSchemaToZod(schema: PlainObject): ZodSchema {
      ...
      for (const [key, val] of Object.entries(schema.properties)) {
        ...
        switch (type) {
          ...
          case 'array': {
            ...
            switch (itemType) {
              case 'string':
                zodItemType = z.string();
                break;
              case 'number':
                zodItemType = z.number();
                break;
              case 'boolean':
                zodItemType = z.boolean();
                break;
              case 'object':
                zodItemType = this.convertJsonSchemaToZod(propSchema.items);
                break;
              default:
                zodItemType = z.any();
            }
    ...
    throw new Error(
      'Invalid schema definition: expected type=object with properties',
    );
  }
}
